...
{ 
  ...
  this.ahead = new Vec3(0.0, 0.0, -1.0); //-z, in right hand system, z pointing forward
  this.right = new Vec3(1.0, 0.0, 0.0); //x
  this.up = new Vec3(0.0, 1.0, 0.0);  //y
...
PerspectiveCamera.prototype.updateViewMatrix = function(){ 
  this.viewMatrix.set( 
   ...
   -this.ahead.x          , -this.ahead.y      ,  -this.ahead.z      , 0, 
    ...
    //takes whatever want to see and puts in in screen space
...
PerspectiveCamera.prototype.move = function(dt, keysPressed) { 
  //change yaw, pitch on mouse drag
  if(this.isDragging){ 
    ...
    this.mouseDelta = new Vec2(0.0, 0.0);
    //base directions from yaw, pitch angles
    this.ahead = new Vec3(
...
//Event handlers, do not forget to call them
